...
@tool
def validate_and_fix_companies(company_names: list, category: str) -> str:
    """
    Valida la lista de nombres de empresa y activa fallbacks automáticos
    para garantizar exactamente 5 nombres reales y limpios.

    Usa guardrails en 3 capas:
      1. Validación léxica (longitud, stopwords, patrones inválidos)
      2. Búsqueda directa en Serper si quedan < 3 válidas
      3. Llamada directa a Claude API con prompt JSON estricto si aún faltan

    Args:
        company_names: Lista de candidatos devuelta por el agente (puede tener basura).
        category:      Categoría de búsqueda original (ej: 'Neobancos en España').

    Returns:
        Cadena con los 5 nombres finales, uno por línea numerada.
    """
    from company_agent.tools.company_guardrails import ensure_five_companies
    final = ensure_five_companies(raw_names=list(company_names), category=category)
    lines = [f"{i+1}. {n}" for i, n in enumerate(final)]
    return "Empresas validadas:\n" + "\n".join(lines)
